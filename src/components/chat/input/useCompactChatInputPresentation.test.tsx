...
describe('useCompactChatInputPresentation', () => {
  it('returns compact for single line', async () => {
    ...
    frame.innerHTML = '<div data-composer-compact-row style="width:200px"><div class="composer-tiptap" style="width:100px"></div></div>';
    document.body.appendChild(frame);
    const ref = { current: frame as unknown as HTMLDivElement } as React.RefObject<HTMLDivElement>;
    const { result } = renderHook(() => useCompactChatInputPresentation({ enabled: true, frameRef: ref, isComposing: () => false }));
