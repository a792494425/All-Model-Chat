...
describe('MessageActions', () => {
  ...
  it('does not render variant switcher when message has no variants or only one variant', () => {
    act(() => {
      renderMessageActions({
        message: {
          ...message,
    ...
    expect(renderer.container.querySelector('[data-testid="message-variant-switcher"]')).not.toBeInTheDocument();
  ...
  it('renders variant switcher and switches variants when clicked', () => {
    const onSwitchVariant = vi.fn();
    const v1: ChatMessage = { id: 'm-1', role: 'model', content: 'Response 1', timestamp: new Date() };
    const v2: ChatMessage = { id: 'm-2', role: 'model', content: 'Response 2', timestamp: new Date() };
    const v3: ChatMessage = { id: 'm-3', role: 'model', content: 'Response 3', timestamp: new Date() };

    // Test at index 1 (middle version: 2 / 3)
    act(() => {
      renderMessageActions({
        message: {
          ...message,
    ...
    // Click Prev -> should call onSwitchVariant('message-1', 0)
    act(() => {
      prevBtn?.dispatchEvent(new MouseEvent('click', { bubbles: true }));
    });
    expect(onSwitchVariant).toHaveBeenCalledWith('message-1', 0);

    // Click Next -> should call onSwitchVariant('message-1', 2)
    act(() => {
      nextBtn?.dispatchEvent(new MouseEvent('click', { bubbles: true }));
    });
    expect(onSwitchVariant).toHaveBeenCalledWith('message-1', 2);
  });
});
