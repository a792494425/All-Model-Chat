    ...
    <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div className={`${SETTINGS_SECTION_CARD_CLASS} p-3 flex items-center justify-between gap-3`}>
        <h4 className="text-sm font-semibold text-[var(--theme-text-primary)] shrink-0">
          {t('shortcutsGeneralTitle' as never) !== 'shortcutsGeneralTitle'
            ? (t('shortcutsGeneralTitle' as never) as string)
            : 'Keyboard Shortcuts'}
          <span className="ml-2 text-xs font-normal text-[var(--theme-text-secondary)]">
          ...
          <div className="relative flex items-center gap-1.5 rounded-lg border border-[var(--theme-border-secondary)] bg-[var(--theme-bg-primary)] px-2 h-8 text-xs">
            ...
            >
              ...
              <option value="general">
                {getCategoryLabel('general')} ({countByCategory.general})
              </option>
              <option value="input">
                {getCategoryLabel('input')} ({countByCategory.input})
              </option>
              <option value="global">
                {getCategoryLabel('global')} ({countByCategory.global})
              </option>
            </select>
          </div>
      ...
      <div className={`${SETTINGS_SECTION_CARD_CLASS} p-0 overflow-hidden`}>
        {filteredItems.length === 0 ? (
          <div className="py-10 text-center text-sm text-[var(--theme-text-secondary)]">
            {t('shortcutsEmpty' as never) || 'No shortcuts found'}
          </div>
        ) : (
                ...
                <React.Fragment key={item.id}>
                  ...
                  >
                    <div className="min-w-0 pr-2">
                      <div className="truncate text-[14px] font-medium text-[var(--theme-text-primary)]">
