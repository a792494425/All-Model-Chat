const dataSettings = {
  ...
  settingsReset: { en: 'Reset Settings Only', zh: '仅重置设置', ja: '設定のみリセット', ko: '설정만 초기화', es: 'Restablecer solo ajustes', fr: 'Réinitialiser uniquement les paramètres', de: 'Nur Einstellungen zurücksetzen' },
  ...
  settingsClearCacheConfirm: {
    ...
    zh: '您确定要清除所有应用数据吗？\n\n这将删除：\n- 已保存的设置\n- 聊天记录\n- 预加载的场景\n\n此操作无法撤销。',
    ja: 'すべてのアプリデータを削除してもよろしいですか？\n\n以下が削除されます：\n- 保存された設定\n- チャット履歴\n- プリロードされたシナリオ\n\nこの操作は元に戻せません。', ko: '모든 애플리케이션 데이터를 지우시겠습니까?\n\n다음 항목이 제거됩니다:\n- 저장된 설정\n- 채팅 기록\n- 미리 로드된 시나리오\n\n이 작업은 되돌릴 수 없습니다.', es: '¿Seguro que quieres borrar todos los datos de la aplicación?\n\nEsto eliminará:\n- Ajustes guardados\n- Historial de chats\n- Escenarios precargados\n\nEsta acción no se puede deshacer.', fr: 'Voulez-vous vraiment effacer toutes les données de l\'application ?\n\nCela supprimera :\n- Paramètres enregistrés\n- Historique des discussions\n- Scénarios préchargés\n\nCette action est irréversible.', de: 'Möchten Sie wirklich alle Anwendungsdaten löschen?\n\nDadurch werden entfernt:\n- Gespeicherte Einstellungen\n- Chatverlauf\n- Vorgeladene Szenarien\n\nDiese Aktion kann nicht rückgängig gemacht werden.' },
  settingsInstallApp: { en: 'Install App', zh: '安装应用', ja: 'アプリをインストール', ko: '앱 설치', es: 'Instalar aplicación', fr: 'Installer l\'application', de: 'App installieren' },
  settingsInstallAppAria: { en: 'Install Progressive Web App', zh: '安装渐进式网络应用', ja: 'プログレッシブウェブアプリをインストール', ko: '프로그레시브 웹 앱 설치', es: 'Instalar aplicación web progresiva', fr: 'Installer l\'application web progressive', de: 'Progressive Web App installieren' },
  settingsInstallAppUnavailableTitle: {
    en: 'App is already installed or not supported by your browser.',
    zh: '应用已安装或浏览器不支持安装。',
    ja: 'アプリは既にインストールされているか、お使いのブラウザではサポートされていません。', ko: '앱이 이미 설치되어 있거나 브라우저에서 지원하지 않습니다.', es: 'La aplicación ya está instalada o tu navegador no la admite.', fr: 'L\'application est déjà installée ou n\'est pas prise en charge par votre navigateur.', de: 'Die App ist bereits installiert oder wird von deinem Browser nicht unterstützt.' },
  settingsInstallAppManualTitle: {
    en: 'Use your browser menu to install this app.',
    zh: '请使用浏览器菜单将此应用安装到设备。',
    ja: 'ブラウザのメニューからこのアプリをインストールしてください。', ko: '브라우저 메뉴를 사용하여 이 앱을 설치하세요.', es: 'Usa el menú de tu navegador para instalar esta aplicación.', fr: 'Utilisez le menu de votre navigateur pour installer cette application.', de: 'Verwende das Browsermenü, um diese App zu installieren.' },
  settingsImportHistory: { en: 'Import History', zh: '导入历史', ja: '履歴をインポート', ko: '기록 가져오기', es: 'Importar historial', fr: 'Importer l\'historique', de: 'Verlauf importieren' },
  ...
  settingsImportError: {
    ...
    ja: '設定のインポートに失敗しました。ファイルが破損しているか、形式が正しくない可能性があります。', ko: '설정을 가져오지 못했습니다. 파일이 손상되었거나 형식이 잘못되었을 수 있습니다.', es: 'No se pudieron importar los ajustes. El archivo podría estar dañado o tener un formato incorrecto.', fr: 'Échec de l\'importation des paramètres. Le fichier est peut-être corrompu ou dans un mauvais format.', de: 'Einstellungen konnten nicht importiert werden. Die Datei ist möglicherweise beschädigt oder hat ein falsches Format.' },
  settingsImportErrorWithMessage: { en: 'Import failed: {message}', zh: '导入失败：{message}', ja: 'インポートに失敗しました：{message}', ko: '가져오기 실패: {message}', es: 'Error al importar: {message}', fr: 'Échec de l\'importation : {message}', de: 'Import fehlgeschlagen: {message}' },
  settingsImportInvalidFileFormat: {
    ...
    ja: 'ファイル形式が無効です。{expectedType}が必要ですが、{foundType}が見つかりました。', ko: '잘못된 파일 형식입니다. {expectedType}이어야 하는데 {foundType}이(가) 발견되었습니다.', es: 'Formato de archivo no válido. Se esperaba {expectedType}, se encontró {foundType}.', fr: 'Format de fichier invalide. Attendu : {expectedType}, trouvé : {foundType}.', de: 'Ungültiges Dateiformat. Erwartet {expectedType}, gefunden {foundType}.' },
  settingsImportHistoryInvalidData: {
    en: 'History data is missing or not an array.',
    zh: '历史数据缺失或不是数组。',
    ja: '履歴データが見つからないか、配列ではありません。', ko: '기록 데이터가 없거나 배열이 아닙니다.', es: 'Faltan datos del historial o no son una matriz.', fr: 'Les données d\'historique sont manquantes ou ne sont pas un tableau.', de: 'Verlaufsdaten fehlen oder sind kein Array.' },
  ...
  settingsLocalAppDataError: {
    ...
    zh: '暂时无法估算本地存储占用。',
    ja: '現在、ローカルストレージの使用量を推定できません。', ko: '지금은 로컬 저장소 사용량을 추정할 수 없습니다.', es: 'No se puede estimar el uso del almacenamiento local en este momento.', fr: 'Impossible d\'estimer l\'utilisation du stockage local pour le moment.', de: 'Die lokale Speichernutzung kann derzeit nicht geschätzt werden.' },
  settingsClearLogs: { en: 'Clear Logs', zh: '清空日志', ja: 'ログをクリア', ko: '로그 지우기', es: 'Borrar registros', fr: 'Effacer les journaux', de: 'Protokolle löschen' },
