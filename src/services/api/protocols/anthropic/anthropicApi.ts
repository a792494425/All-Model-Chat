...
export const generateAnthropicTurnStreamApi = async (
  apiKey: string,
  modelId: string,
  contents: ChatHistoryItem[],
  config: unknown,
  ...
  const anthropicConfig = asAnthropicChatConfig(config);
  const url = buildAnthropicMessagesUrl(anthropicConfig.baseUrl);
  const requestBody = buildAnthropicRequestBody(modelId, contents, [], anthropicConfig, 'user', true);
  const requestInit = createRequestInit(
    apiKey,
    requestBody,
    abortSignal,
  ...
  await readAnthropicStreamEvents(response, abortSignal, (event: AnthropicStreamEvent) => {
    ...
    if (event.type === 'content_block_delta') {
      ...
      if (delta?.text) {
        text += delta.text;
        streamCallbacks?.onPart?.({ text: delta.text });
      }
      if (delta?.type === 'thinking_delta' && delta.thinking) {
        thoughts += delta.thinking;
        streamCallbacks?.onThoughtChunk?.(delta.thinking);
      }
